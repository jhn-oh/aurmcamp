...
def lookup(a, b):
  ...
  if [a, b] == [4, 4]:
    ...
    checklist = []
    matrix[a, b] = 0
    if 0 <= a <= 4 and 0 <= b-1 <= 4 and matrix[a, b-1] == 1: #왼쪽으로
        lookup_return.append([a, b-1])
    elif 0 <= a+1 <= 4 and 0 <= b <= 4 and matrix[a+1, b] == 1: #아래로
        lookup_return.append([a+1, b])
    elif 0 <= a-1 <= 4 and 0 <= b <= 4 and matrix[a-1, b] == 1: #위로
        lookup_return.append([a-1, b])
    elif 0 <= a <= 4 and 0 <= b+1 <= 4 and matrix[a, b+1] == 1: #오른쪽으로
        lookup_return.append([a, b+1])
    ...
    elif len(lookup_return) == 1: #정상
      lookup(lookup_return[0][0], lookup_return[0][1])
    elif len(lookup_return) == 0 and len(checkpoint_list) == 2: : #막다른 길
